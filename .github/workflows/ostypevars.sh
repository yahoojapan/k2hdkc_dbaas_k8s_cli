...
#---------------------------------------------------------------------
if [ "X${CI_OSTYPE}" = "Xubuntu:20.04" ] || [ "X${CI_OSTYPE}" = "Xubuntu:focal" ]; then
...
elif [ "X${CI_OSTYPE}" = "Xubuntu:18.04" ] || [ "X${CI_OSTYPE}" = "Xubuntu:bionic" ]; then
...
elif [ "X${CI_OSTYPE}" = "Xubuntu:16.04" ] || [ "X${CI_OSTYPE}" = "Xubuntu:xenial" ]; then
...
elif [ "X${CI_OSTYPE}" = "Xdebian:10" ] || [ "X${CI_OSTYPE}" = "Xdebian:buster" ]; then
...
elif [ "X${CI_OSTYPE}" = "Xdebian:9" ] || [ "X${CI_OSTYPE}" = "Xdebian:stretch" ]; then
...
elif [ "X${CI_OSTYPE}" = "Xcentos:7" ] || [ "X${CI_OSTYPE}" = "Xcentos:centos7" ]; then
	DIST_TAG="el/7"
	INSTALL_PKG_LIST="git autoconf automake gcc-c++ make pkgconfig redhat-rpm-config rpm-build ruby-devel rubygems procps openssl"
	CONFIGURE_EXT_OPT=""
	INSTALLER_BIN="yum"
	INSTALL_QUIET_ARG=""
	PKG_TYPE_DEB=0
	...
	# For ShellCheck
	#
	"${INSTALLER_BIN}" install -y "${INSTALL_QUIET_ARG}" epel-release
	yum-config-manager --disable epel
	"${INSTALLER_BIN}" --enablerepo=epel install -y "${INSTALL_QUIET_ARG}" ShellCheck
